...
dataset_name = 'AmazonFashion_Byteioimage.npy'
...
dataset = np.load('../'+dataset_name,encoding="latin1")
...
#initialize the weights and biases(set initial variable for tensorflow)
def Weights(name):
...
with tf.device('/gpu:0'):
    ...
    cost_train = tf.reduce_sum(tf.log(tf.sigmoid(tf.reduce_sum(tf.multiply(tf.gather(thetau,u),tf.subtract(result1,result2)),1,keepdims=True))))
...
def Evaluation(step):
    ...
    for i in range(len(idx)):
        cc=0
        for j in idx[i]:
            input_images[cc]=np.uint8(np.asarray(Image.open(Item[j]['imgs']).convert('RGB').resize((224,224))))
            cc+=1
...
#随机选取一个用户u，然后获取他所有买过的产品，并随机选择一个得到物品i，再随机选取一个没买过的物品j
def sample(user):
    ...
    while True:
        ...
        if (not j in M): break
    print("sample:user"+str(u)+" with item:"+str(i)+" to item:"+str(j))
    return (u,i,j)

#不停的随机选取u,i,j，并加载其图片，输入进TensorFlow的输入流里
def load_image_async():
    while True:
        (uuu,iii,jjj)=sample(user_train)
        jpg1=np.uint8(np.asarray(Image.open(Item[iii]['imgs']).convert('RGB').resize((224,224))))
        jpg2=np.uint8(np.asarray(Image.open(Item[jjj]['imgs']).convert('RGB').resize((224,224))))
